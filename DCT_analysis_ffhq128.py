...
Y_bound = np.full((16,), 480.25)

Cb_bound = np.full((16,), 480.25)

Cr_bound = np.full((16,), 480.25)


Y_std = np.array(
    [6.53, 3.478, 2.196, 1.405, 3.378, 2.381, 1.765, 1.185, 2.072, 1.697, 1.291, 0.999, 1.291, 1.083, 0.999, 0.999]
)

Cb_std = np.array(
    [4.138, 1.741, 0.999, 0.998, 1.589, 0.998, 0.998, 0.998, 0.998, 0.998, 0.997, 0.998, 0.997, 0.998, 0.998, 0.998]
)

Cr_std = np.array(
    [4.391, 1.904, 0.998, 0.998, 1.758, 1.082, 0.998, 0.998, 0.999, 0.998, 0.998, 0.998, 1.0, 0.998, 0.998, 0.998]
)
...
def resize_ffhq1256_to_ffhq128(ffhq256_folder=None, output_folder=None):
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)

    cnt = 0
    for filename in os.listdir(ffhq256_folder):
        if filename.endswith(('.png', '.jpg', '.jpeg', '.bmp')):  # Check for image files
            img_path = os.path.join(ffhq256_folder, filename)
            img = Image.open(img_path)
            img = img.resize((128, 128))  # Resize the image
            output_path = os.path.join(output_folder, filename)
            img.save(output_path)  # Save the resized image
            cnt += 1
    print(f"{cnt} imgs saved into {output_folder}")


def image_to_DCT_array(img_folder=None, block_sz=4, coe=None):
    ...
    if coe.lower() == 'y':
        ...
        np.save(f'/home/mang/Downloads/ffhq128_{block_sz}by{block_sz}_y', Y_coe)
    ...
    if coe.lower() == 'cb':
        ...
        np.save(f'/home/mang/Downloads/ffhq128_{block_sz}by{block_sz}_cb', Cb_coe)
        ...
        np.save(f'/home/mang/Downloads/ffhq128_{block_sz}by{block_sz}_cr', Cr_coe)
...
if __name__ == "__main__":
    ...
    # resize_ffhq1256_to_ffhq128(ffhq256_folder='/home/mang/Downloads/ffhq256_jpg/ffhq256',
    #                            output_folder='/home/mang/Downloads/ffhq128_jpg/ffhq128')
    # image_to_DCT_array(img_folder='/home/mang/Downloads/ffhq128_jpg/ffhq128',
    #                    block_sz=4, coe='cr')
    DCT_statis_from_array(array_path='/home/mang/Downloads/ffhq128_4by4_cr.npy',
