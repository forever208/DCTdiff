    ...
    def __init__(self,
        ...
        if add_downsample:
            self.downsample = DownSampleBlock2D(
                in_channels=out_channels, downsample_factor=downsample_factor, kernel_size=downsample_kernel_size
            )
...
class VAEEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_in = nn.Conv2d(
            in_channels=in_channels, out_channels=self.channels_per_block[0], kernel_size=3, stride=1, padding=1
        )
        ...
        self.out_norm = nn.GroupNorm(
            num_channels=self.channels_per_block[-1], num_groups=groupnorm_groups, eps=1e-6)

        conv_out_channels = 2 * self.latent_channels if double_z else self.latent_channels
        self.conv_out = nn.Conv2d(self.channels_per_block[-1], conv_out_channels, kernel_size=3, padding="same")  # We want 4 latent channels (so 4 for mean and 4 for std)
...
class VAEDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        ### Final Output Layers ###
        self.out_norm = nn.GroupNorm(num_channels=self.channels_per_block[-1], num_groups=groupnorm_groups, eps=1e-6)
        self.conv_out = nn.Conv2d(
            self.channels_per_block[-1], self.out_channels, kernel_size=3, padding="same")
...
class VAE(EncoderDecoder):
    ...
    def __init__(self, config):
        ...
        self.config = config
        self.eval()  # model frozen when training the diffusion
        self.requires_grad_(False)  # model frozen when training the diffusion
